...
| `rectangle` | Window tiling/management | |
| `sublime-text` | Text editor | |
| `spokenly` | Speech-to-text dictation | |
...
- By 20/08/2026, moved off pCloud: this repo was intended to live at
  `~/Documents/My_Code/dotfiles` (the old pCloud copy had sync-conflict
  damage to its `.git`), but ended up at `/Volumes/MainDataDrive/My_Code/dotfiles`
  instead (see the entry below) - that `~/Documents` location was never
...
- By 20/08/2026, folded the separate `automation/workflow` repo's
  ...
  wasn't carried over.
- By 20/08/2026, on the `imac-version` branch: merged the above changes in
  from `main`, keeping this branch's iMac-specific `nix/configuration.nix`
  values (`x86_64-darwin`, `_HIHideMenuBar = true`, and its own homebrew
  brews/casks list) rather than main's MacBook-specific ones. Also moved this
  repo (again) to `/Volumes/MainDataDrive/My_Code/dotfiles`, off the
  Dropbox-synced `.../Dropbox/02 - Areas/My_Code/dotfiles` path, to avoid the
